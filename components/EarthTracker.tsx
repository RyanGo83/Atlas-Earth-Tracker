...
import { 
  Trash2, PlusCircle, Globe2, Trophy, ArrowUp, ArrowDown, Minus, Crown, Target, 
...
interface AllEarthData {
  regions: Record<string, EarthData>;
...
const STORAGE_KEY = 'atlas_earth_data_v2';
...
export const EarthTracker: React.FC = () => {
  const [data, setData] = useState<AllEarthData>({ regions: {}, currentRegion: null });
  ...
  const [potentialRegions, setPotentialRegions] = useState<string[]>([]);
  ...
  const addRegion = () => {
    ...
    if (data.regions[newRegionName]) { alert('Region exists'); return; }
    
    setData(prev => ({
      ...prev,
      regions: { ...prev.regions, [newRegionName]: { entries: [], lastUpdated: new Date().toISOString() } },
      currentRegion: newRegionName
    }));
    setNewRegionName('');
  };

  const deleteCurrentRegion = () => {
    if (!data.currentRegion) return;
    if (!window.confirm(`Delete ${data.currentRegion}?`)) return;
    
    const newRegions = { ...data.regions };
    delete newRegions[data.currentRegion];
    const nextRegion = Object.keys(newRegions).sort()[0] || null;
    setData({ regions: newRegions, currentRegion: nextRegion });
  };
  ...
  const handleSaveEntry = () => {
    ...
    const currentList = data.regions[data.currentRegion].entries;
    let updatedEntries;
    if (editingId) {
    ...
    setData(prev => ({
      ...
      regions: {
        ...prev.regions,
        [prev.currentRegion!]: { entries: updatedEntries, lastUpdated: new Date().toISOString() }
      }
    }));
  ...
  const startEditing = (entry: EarthEntry) => {
  ...
  const openManageModal = () => {
        ...
        if (parsed.rivals) {
          Object.values(parsed.rivals).forEach((entries: any) => {
            entries.forEach((e: any) => {
              if (e.entryType === 'EARTH' && e.townName) {
    ...
    setPotentialRegions(sorted);
    
    const currentSelection = new Set<string>();
    Object.keys(data.regions).forEach(t => currentSelection.add(t));
    setSelectedRegions(currentSelection);
  ...
  const executeManage = () => {
    ...
    selectedRegions.forEach(t => {
      if (!newRegions[t]) {
        newRegions[t] = { entries: [], lastUpdated: new Date().toISOString() };
        changes = true;
      }
    });
    
    potentialRegions.forEach(t => {
      if (!selectedRegions.has(t) && newRegions[t]) {
        delete newRegions[t];
        changes = true;
      }
    });
    ...
    if (changes) {
      let next = data.currentRegion;
      if (data.currentRegion && !newRegions[data.currentRegion]) {
        next = Object.keys(newRegions).sort()[0] || null;
      }
      setData(prev => ({ ...prev, regions: newRegions, currentRegion: next || prev.currentRegion }));
    }
  ...
  const currentEntries = data.currentRegion ? data.regions[data.currentRegion].entries : [];

  const playerHistory = useMemo(() => {
    const history: Record<string, EarthEntry[]> = {};
  ...
  const getRank = (list: EarthEntry[], name: string) => {
  ...
  const worldLeaderEntry = currentLeaderboard.length > 0 ? currentLeaderboard[0] : null;

  let parcelsForWorldLeader = 0;
  let parcelsForNextRank = 0;
  let nextRankName = "";

  if (myEntry) {
    if (worldLeaderEntry && myEntry.name !== worldLeaderEntry.name) {
      parcelsForWorldLeader = (worldLeaderEntry.parcels - myEntry.parcels) + 1;
    }
  ...
  } else if (worldLeaderEntry) {
    parcelsForWorldLeader = worldLeaderEntry.parcels + 1;
  }
    ...
    <div className="space-y-6">
          ...
          <div className="bg-slate-800 rounded-xl border border-slate-600 shadow-2xl w-full max-w-2xl flex flex-col max-h-[80vh]">
            <div className="p-4 border-b border-slate-700 flex justify-between items-center bg-slate-900/50 rounded-t-xl">
              <div>
                <h3 className="text-xl font-bold text-white flex items-center gap-2">
                  <Users className="text-cyan-400" /> Manage Earth Regions
                </h3>
                <p className="text-xs text-slate-500">Import regions from your Rival Tracker entries.</p>
              </div>
            ...
            <div className="p-4 border-b border-slate-700 bg-slate-900/20">
              <div className="relative">
                ...
                <input 
                  type="text" 
                  placeholder="Search potential regions..." 
                  value={modalSearch}
                  onChange={e => setModalSearch(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-700 rounded-lg pl-10 pr-4 py-2 text-white outline-none focus:border-cyan-500"
                />
              </div>
            </div>

            <div className="p-4 overflow-y-auto flex-1 space-y-2 bg-slate-900/30">
              {potentialRegions.filter(t => t.toLowerCase().includes(modalSearch.toLowerCase())).length === 0 ? (
                <div className="text-center p-12 text-slate-500">
                  <Search className="mx-auto opacity-10 mb-2" size={48} />
                  <p>No new regions found in Rival Tracker.</p>
                </div>
              ) : (
                potentialRegions.filter(t => t.toLowerCase().includes(modalSearch.toLowerCase())).map(t => {
                  const isSelected = selectedRegions.has(t);
                  return (
                    <div 
                      key={t} 
                      className={`p-3 rounded-lg border flex items-center justify-between transition-all cursor-pointer ${isSelected ? 'bg-cyan-900/30 border-cyan-500/50 shadow-inner' : 'bg-slate-700/30 border-slate-600 hover:border-slate-500 hover:bg-slate-700/50'}`}
                      onClick={() => toggleSelection(t)}
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-6 h-6 rounded-md border flex items-center justify-center transition-all ${isSelected ? 'bg-cyan-500 border-cyan-500' : 'border-slate-500 bg-slate-800'}`}>
                          {isSelected && <Check size={16} className="text-white" />}
                        </div>
                        <span className={`font-bold ${isSelected ? 'text-white' : 'text-slate-300'}`}>{t}</span>
                      </div>
                      {isSelected && <span className="text-[10px] font-bold text-cyan-400 uppercase">Tracked</span>}
                    </div>
                  );
                })
              )}
            ...
            <div className="p-4 border-t border-slate-700 bg-slate-900/50 rounded-b-xl flex justify-between items-center">
              <span className="text-xs text-slate-400 font-mono font-bold">{selectedRegions.size} regions selected</span>
              <div className="flex gap-2">
                ...
                <button onClick={executeManage} className="px-6 py-2 rounded-lg font-bold text-white bg-cyan-600 hover:bg-cyan-500 shadow-xl transition-all active:scale-95">Save Changes</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 shadow flex flex-col md:flex-row gap-4 items-center justify-between">
         <div className="flex items-center gap-3 w-full md:w-auto">
            <Globe2 className="text-cyan-400 flex-shrink-0" />
            <select 
              className="bg-slate-900 border border-slate-600 rounded p-2 text-white font-bold cursor-pointer outline-none focus:ring-2 focus:ring-cyan-500 flex-1 md:flex-none"
              value={data.currentRegion || ''}
              onChange={(e) => setData(prev => ({ ...prev, currentRegion: e.target.value }))}
            >
              {Object.keys(data.regions).sort().map(t => <option key={t} value={t}>{t}</option>)}
            </select>
         </div>
         ...
         <div className="flex gap-2 w-full md:w-auto items-center">
            <button 
              ...
              className="flex items-center gap-2 bg-slate-700 hover:bg-slate-600 text-cyan-400 py-2 px-3 rounded text-xs font-bold uppercase border border-cyan-500/20 transition-colors"
            >
              <Users size={16} /> Manage Regions
            </button>
            ...
            <input 
              ...
              className="bg-slate-900 border border-slate-600 rounded p-2 text-sm text-white w-full md:w-40 outline-none focus:border-cyan-500"
            />
            <button onClick={addRegion} className="bg-green-600 hover:bg-green-500 text-white p-2 rounded shrink-0"><PlusCircle size={18}/></button>
            <button onClick={deleteCurrentRegion} className="bg-red-600 hover:bg-red-500 text-white p-2 rounded shrink-0"><Trash2 size={18}/></button>
         </div>
      </div>

      {data.currentRegion && worldLeaderEntry && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gradient-to-br from-yellow-900/40 to-slate-800 p-4 rounded-xl border border-yellow-500/30 flex items-center gap-4 shadow-lg">
             ...
             <div>
                <div className="text-yellow-500 text-xs font-bold uppercase tracking-wider">World Leader</div>
                <div className="text-white font-bold text-lg truncate max-w-[200px]">{worldLeaderEntry.name}</div>
                <div className="text-slate-300 font-mono text-sm">{worldLeaderEntry.parcels} Parcels</div>
             </div>
          </div>
          ...
          <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 flex items-center gap-4 shadow-lg">
             ...
             <div>
                <div className="text-purple-400 text-xs font-bold uppercase tracking-wider">To Be #1</div>
                {myCurrentRank === 1 ? (
                   <div className="text-green-400 font-bold text-lg">You are #1!</div>
                ) : (
                   <><div className="text-white font-bold text-lg">+{parcelsForWorldLeader} Parcels</div><div className="text-slate-400 text-xs">to take Rank #1</div></>
                )}
             </div>
          </div>
        </div>
      ...
      {data.currentRegion && (
        <div className={`p-6 rounded-xl border shadow transition-all duration-300 ${editingId ? 'bg-slate-800 border-orange-500/50' : 'bg-slate-800 border-slate-700'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
             ...
             <div>
                <input type="text" placeholder="Username" value={entryName} onChange={e => setEntryName(e.target.value)} list="earth-player-names" className="bg-slate-900 border border-slate-600 rounded p-2 text-white outline-none focus:border-blue-500 w-full" />
                <datalist id="earth-player-names">{existingNames.map(name => <option key={name} value={name} />)}</datalist>
             </div>
      ...
      <div className="bg-slate-800 rounded-xl overflow-hidden border border-slate-700 shadow-lg">
        <div className="p-4 bg-slate-900 border-b border-slate-700 flex flex-col sm:flex-row justify-between items-center gap-4">
           <span className="text-white font-bold flex items-center gap-2"><Globe2 size={16} /> Leaderboard History: {data.currentRegion}</span>
           <div className="flex items-center gap-4">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-700">
                    {currentLeaderboard.length === 0 ? (
                      <tr><td colSpan={5} className="p-8 text-center text-slate-500">No data for this region yet.</td></tr>
                    ) : (
